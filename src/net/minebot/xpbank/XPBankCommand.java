...
public class XPBankCommand implements CommandExecutor {
	...
	public XPBankCommand(MinebotXPBank plugin) {
		//this.plugin = plugin;
	}
	...
	public boolean onCommand(CommandSender sender, Command command,
		...
		if (sender instanceof Player) {
			if (!((Player)sender).hasPermission("xpbank.use")) {
				return false;
			}
		}
	...
	private void commandBalance(CommandSender sender, String[] args) {
		...
		if (target != null) {
			sender.sendMessage(ChatColor.GREEN + "XP on hand: " + ChatColor.WHITE + MinebotXPBank.calculateXP(target) + " XP ");
		}
	}
